...
<div align="center" id="div1" style="display: none"> 
	<div align="center" id="div3" style="position:relative;margin-top:50px;display: block">
		<div id="SbTitle" style="position:relative;margin-bottom:20px;"><font size="5">Y20XX Social Insurance Base Confirm</font></div>
		<table id="tbSb" style="width:100%;" border="0" cellspacing="1" cellpadding="0" class="thin">
			<tr><td>EmployeeName</td><td>DeptName</td><td>ID Code</td><td id="SbTitleAVIncomeEg">Y20XX-1_Average Monthly Salary</td><td id="SbTitlebaseEg">Y20XX_Social Insurance Base</td><td>Confirm by Employee</td></tr>	
			<tr><td>姓名</td><td>部门</td><td>身份证号码</td><td id="SbTitleAVIncomeCh">20XX-1年月平均收入</td><td id="SbTitlebaseCh">20XX年社保基数</td><td>员工确认</td></tr>
			<tr><td class="name"></td><td class="dept"></td><td class="idcode"></td><td class="incomeAvg"></td><td id="Sb"></td><td class="emp"></td></tr>
		<table>
		<br>
		<input style="height:5%;" id="printing" type="button" value="导出/export" onclick ="pringting()"/>
...
function querygjj(){
	...
	jQuery.ajax({ 
		...
		success: function(data) {
			jQuery.each(data,function(id,item){
				if(item.error=="true"){
					jQuery("#SbTitle").html("Y"+(parseInt(cxyear)+1)+" Social Insurance Base Confirm");
					jQuery("#SbTitleAVIncomeEg").html("Y"+cxyear+"_Average Monthly Salary");
					jQuery("#SbTitlebaseEg").html("Y"+(parseInt(cxyear)+1)+"_Social Insurance Base");
					jQuery("#SbTitleAVIncomeCh").html(cxyear+"年月平均收入");
					jQuery("#SbTitlebaseCh").html((parseInt(cxyear)+1)+"年社保基数");
...
function pringting(){
	...
	jQuery.ajax({ 
		url: '/socialSecPdfSession', 
		type: 'post', 
		data:{hid_msg:hid_msg},
		dataType: 'text',
		async: false,
		success: function(data) {
			window.location.href='/socialSecurityReport';
		}, 
		error: function(data) {
			top.Dialog.alert("error");
		} 
